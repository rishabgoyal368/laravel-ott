...
@section('title','All Manual Payment Transacation')
@section('content')
          ...
          <tbody>
            @foreach($manual_payment as $key=>$payment)
              <tr>
                ...
                <td>
                  @if($payment->status == 1)
                    <a href="{{url('manualpayment',$payment->id)}}" class='btn btn-sm btn-success'>Active</a>
                  @else
                    <a href="{{url('manualpayment',$payment->id)}}" class='btn btn-sm btn-danger'>Deactive</a> 
                  @endif
                </td>
                <td>
