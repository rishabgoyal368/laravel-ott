 ...
 @if($moviegenreitems != NULL && count($moviegenreitems)>0)
     <h5 class="section-heading">{{$genre->name }} in {{ $menu->name }}</h5>
      ...
      @if($auth && $subscribed==1)
        ...
        <a href="{{ route('show.in.genre',$genre->id) }}" class="see-more"> <b>{{__('staticwords.viewall')}}</b></a>
     
      @else
      
        <a href="{{ route('show.in.guest.genre',$genre->id) }}" class="see-more"> <b>{{__('staticwords.viewall')}}</b></a>
