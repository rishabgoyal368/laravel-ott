  ...
  <div class="background-main-poster-overlay">
    ...
    @if(isset($liveevent))
      @if($liveevent->poster != null)
        <div class="background-main-poster col-md-offset-4 col-md-6" style="background-image: url('{{asset('images/event/posters/'.$liveevent->poster)}}');">
      @else
              ...
              <div class="">
                <ul class="casting-headers">
                  ...
                  @if($today_date >= $start_date && $today_date <= $end_date)
                    ...
                    @if($liveevent->video_link['iframeurl'] != null)
                      @if(Auth::check() && Auth::user()->is_admin == '1')
                        <a onclick="playoniframe('{{ $liveevent['iframeurl'] }}','{{ $liveevent->id }}','event')" class="btn btn-play play-btn-icon{{ $liveevent['id'] }}"><span class="play-btn-icon"><i class="fa fa-play"></i></span> <span class="play-text">{{__('staticwords.playnow')}}</span>
                         </a>
                      @else
                    ...
                    @else
                      @if(Auth::check() && Auth::user()->is_admin == '1')
                        <a  href="{{route('watchevent',$liveevent->id)}}" class="iframe btn btn-play play-btn-icon{{ $liveevent['id'] }}"><span class="play-btn-icon"><i class="fa fa-play"></i></span> <span class="play-text">{{__('staticwords.playnow')}}</span>
                          </a>
                      @else
                        <a href="{{url('/watch/event/'.$liveevent->id)}}" class="iframe btn btn-play play-btn-icon{{ $liveevent['id'] }}"><span class="play-btn-icon "><i class="fa fa-play" ><span class="play-text"> {{__('staticwords.playnow')}}</span></a>
                      @endif
                    @endif
