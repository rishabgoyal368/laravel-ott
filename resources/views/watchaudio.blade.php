...
<head>
	<title>{{__('staticwords.watchmovie')}} - {{ $audio->title }}</title>
...
<body style="background-color:#999999; padding:0px; margin:0px;">	
	...
	{{-- 	@php
		...
		$checkaudio=Session::get('time_'.$audio_id) ;
		if (!is_null($checkaudio)) {
			$mid=$checkaudio['audio'];
      if ($aid==$audio_id) {
      	$endtime=$checkaudio['endtime'];
      }else{
      	$endtime='00:00:00';
      }
		}else{
		...
		@else
		data-video-source="{{ ($audio->upload_audio )}}"
		@endif data-poster-source="{{asset('images/audio/posters/'.$audio->poster)}}"  data-downloadable="no"  @if(isset($pass) && $pass !=NULL) data-is-private="yes" data-private-video-password="{{ $pass }}" @endif> 
			...
			<div data-video-short-description="">
				<p class="minimalDarkCategoriesTitle"><span class="minimialDarkBold">{{__('staticwords.title')}}: </span>{{ $audio->title }}</p>
				<p class="minimalDarkCategoriesDescription"><span class="minimialDarkBold">{{__('staticwords.description')}}: </span>{{ $audio->detail }}</p>
			</div>
			<div>
				<p class="classicDarkThumbnailTitle">{{__('staticwords.title')}}: </span>{{ $audio->title }}</p>
				<p class="minimalDarkThumbnailDesc">{{__('staticwords.description')}}: </span>{{ $audio->detail }}</p>
			</div>
