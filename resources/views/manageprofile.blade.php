...
@section('title',__('staticwords.manageprofile'))
@section('main-wrapper')
	...
	<div class="container">
		...
		<br>
		<div align="left">
				<h4>{{__('staticwords.hey')}} {{ Auth::user()->name }} {{__('staticwords.selectpersonalprofile')}} {{ config('app.name') }}:</h4>	
		</div>
		<hr>
		<div class="row">
				@if(!isset($result))
				 <p>{{__('staticwords.noprofileavailable')}}</p>
				@endif
			...
			<div class="col-md-3  col-sm-4  col-xs-6">
				...
				<br><br>
					...
					<div class="text-center">
						<br>
						<span class="label label-success">{{ __('staticwords.currentlyactive') }}</span>
					</div>
				...
				@else
					...
					<div class="text-center">
						<br>
						<span class="label label-success">{{ __('staticwords.inuse') }}</span>
					</div>
		  ...
		  @if(isset($result))
			...
			<div class="mg15 col-md-12  col-sm-12 col-xs-12 col-md-offset-5">
				...
				<button type="submit" class="btn btn-lg btn-primary" value="Done"><i class="fa fa-check"></i> {{__('staticwords.done')}}</button>
